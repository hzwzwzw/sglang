...
class _NsaStrategy(StackStrategy):
    def matches(self, kvcache, components):
        from sglang.srt.mem_cache.memory_pool import NSATokenToKVPool

        return isinstance(kvcache, NSATokenToKVPool) and components == {
            ComponentType.FULL
        }
...
class _PlainKvStrategy(StackStrategy):
    def matches(self, kvcache, components):
        ...
        if isinstance(
            kvcache,
            (SWAKVPool, HybridLinearKVPool, NSATokenToKVPool, DeepSeekV4TokenToKVPool),
        ):
            return False
