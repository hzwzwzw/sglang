...
class UnifiedRadixCache(BasePrefixCache):
    ...
    def check_prefetch_progress(self, req_id: str) -> bool:
        ...
        if device_indices is None:
            ...
            page_aligned_len = len(page_aligned_key)
            loaded_from_storage = 0
            if page_aligned_len > 0:
                values = device_indices[:page_aligned_len].to(
                    ...
                    if rec is not None:
                        try:
                            rec.record(
                                ...
                                rid=req_id[-12:],
                                page_aligned_len=page_aligned_len,
                                effective_hit_tokens=effective_hit_tokens,
                                result_prefix_len=getattr(
                                    insert_result, "prefix_len", None
                                ),
                            )
                ...
                # No page-aligned content to insert. Free the slots.
                if device_indices.numel() > 0:
                    self.token_to_kv_pool_allocator.free(device_indices)
                loaded_from_storage = 0
