...
class Scheduler(
    ...
    def abort_request(self, recv_req: AbortReq):
        ...
        # running_mbs / mbs slots, not necessarily self.running_batch (which is
        # the slot for the current mb_id). Scanning only self.running_batch
        # misses the copy when it is staged in another slot, which leaves the
        ...
        # Scan every running_mbs / mbs slot so the AbortReq applies on every
        # stage regardless of which slot holds the copy.
        if self.ps.pp_size == 1:
